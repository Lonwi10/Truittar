...
$_SESSION["tipo1"] = 0;
$_SESSION["tipo2"] = 0;
if(isset($_POST["enviarMen"])){
  enviarMen($_SESSION["Id"], $_POST["mensaje"], $_SESSION["usuario"], $_POST["usrEnviar"]);
}
function enviarMen($id, $mensaje, $emisor, $remitente){

  //definir idioma horario
...
//Buscar el id del remitente
        $queryx = $pdo->prepare("SELECT * FROM usuarios  where nomUsuario ="."'".$remitente."'");
        $queryx->execute();

        $p= $queryx->errorInfo();
        if ($p[0]!='00000') {
          echo "\nPDO::errorInfo():\n";
          die("Error accedint a dades: " . $p[2]);
        }
        $rox = $queryx->fetch();
        $id2 = $rox["ID"];
          
//Insertar mensajes
        $query = $pdo->prepare("INSERT INTO mensajes(emisorId,usEmi,usRem,remitenteId,fecha,mensaje) VALUES (".$id.",'".$emisor."','".$remitente."',".$id2.",'".$hoy."','".$mensaje."')");
        $query->execute();
     ...
     <form action="" method="post">
            ...
            <div class = "postentero">
                ...
                <div class="divpost2"> 
                    ...
                    <input type="text" placeholder="Escribir usuario" name="usrEnviar" class="post"></input>
                    ...
                    <input type="text" placeholder="Escribir mensaje" name="mensaje" class="post"></input> 
                   
                    <br>
                   
                   <button type="submit" class="postear" name="enviarMen"> Enviar </button>
                   <p>
